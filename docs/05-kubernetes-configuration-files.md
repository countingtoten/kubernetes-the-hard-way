...
for instance_id in "${(@f)$(doctl compute droplet list --tag-name worker --no-header --format ID)}"; do
  ...
  scp -i ~/.ssh/id_rsa ${instance}.kubeconfig kube-proxy.kubeconfig root@${ip_address}:~/
done
```
...
for instance_id in "${(@f)$(doctl compute droplet list --tag-name controller --no-header --format ID)}"; do
  ...
  scp -i ~/.ssh/id_rsa admin.kubeconfig kube-controller-manager.kubeconfig kube-scheduler.kubeconfig root@${ip_address}:~/
done
```
